...
| Shape IRI | ndd200a95ffbd4bdf99df60c90fd07e34b118
| Status | `stable` icon:check-circle[]
| Label | system label
| Definition | The primary label or name of the system.
| Scope note | 
| Implementation | A `tern:System` _MUST_ have exactly 1 `rdfs:label` predicate with a string value.
...
| Scope note | 
| Implementation | A `tern:System` _MAY_ have at most 1 `tern:systemType` predicate where the value node is an IRI.
| Cardinality | Maximum 1
| Node kind | link:http://www.w3.org/ns/shacl#IRI[`sh:IRI`]
| Class type | 
| Expected values | 
...
| Scope note | 
| Implementation | If present, `ssn:implements` predicates on a `tern:System` _MUST_ have IRI values of type `tern:Method` or `tern:Procedure`.
| Cardinality | 
| Node kind | link:http://www.w3.org/ns/shacl#IRI[`sh:IRI`]
...
[#class-tern:System-tern:hasAttribute]
